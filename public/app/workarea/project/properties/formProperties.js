export default class FormProperties {
    ...
    show() {
        ...
        setTimeout(() => {
            this.focusTextInput(this.nodeContent.querySelector('input[type="text"'));
        }, 500);
    }
    ...
    combineMetadataProperties() {
        ...
        if (eXeLearning.app.user.preferences.preferences.advancedMode.value == 'true') {
            this.metadataProperties = this.metadataPropertiesBase;
        } else {
            for (let [key, property] of Object.entries(this.metadataPropertiesBase)) {
                if (property.alwaysVisible) this.metadataProperties[key] = property;
            }
        }
    ...
    setBodyElement(bodyElement) {
        this.propertiesFormElement = this.nodeContent.querySelector('#properties-node-content-form');
    ...
    makeBodyElement(properties) {
        ...
        element.classList.add('properties-body-container', 'form-properties', 'loading');
    ...
    addRowsFlatWithSectionTitles(properties, table) {
        ...
        propertiesArray = propertiesArray.sort((a, b) => {
            if (a[1].multipleId && b[1].multipleId && a[1].multipleId === b[1].multipleId) {
                if (a[1].multipleIndex === b[1].multipleIndex) return 0;
                return a[1].multipleIndex > b[1].multipleIndex ? 1 : -1;
            } else if (a[1].multipleId && b[1].multipleId && a[1].prefix === b[1].prefix) {
        ...
        for (const [key, property] of propertiesArray) {
            ...
            if (topGroupKey && !seenTopGroups.has(topGroupKey)) {
                seenTopGroups.add(topGroupKey);
                const hr = document.createElement('hr');
                const groupElementTitle = document.createElement('h5');
                groupElementTitle.classList.add('properties-group-title', 'mb-3');
                let titleText = "<span class='title-text'>" + topGroupTitle + '</span>';
                const catKey = Object.keys(property.category || { '': '' })[0];
                if (catKey == this.cataloguingCategoryKey) {
                    if (property.required) {
                        titleText = '* ' + titleText + " · <span class='required-text'>(" + _('Required') + ')</span>';
                    } else {
                        titleText += " · <span class='optional-text'>(" + _('Optional') + ')</span>';
                    }
                }
                groupElementTitle.innerHTML = "<span class='title'>" + titleText + '</span>';
                table.append(hr);
                table.append(groupElementTitle);
            }

            const propertyRow = this.makeRowElement(key, property);
            if (propertyRow) table.append(propertyRow);
        }
    }

    makeRowElement(name, property) {
        property.id = name;
        const propertyId = property.multipleId ? property.multipleId : property.id;
        const propertyIdGenerated = propertyId + '-' + eXeLearning.app.common.generateId();
        ...
        propertyRow.setAttribute('group', property.groups ? Object.keys(property.groups).pop() : '');
        propertyRow.setAttribute('duplicate', property.duplicate ? property.duplicate : false);

        if (property.multipleId) {
            propertyRow.classList.add('copied-row');
        }

        const propertyTitle = this.makeRowElementLabel(propertyIdGenerated, property);
        const propertyValue = this.makeRowValueElement(propertyIdGenerated, propertyId, property);
        const helpContainer = this.makeRowElementHelp(property);
        const actionsContainer = this.makeRowActionsElement(property, propertyRow);
        if (actionsContainer && property.multipleId) actionsContainer.setAttribute('original', false);
        ...
        if (property.type == 'checkbox') {
            ...
            header.classList.add('header-container', 'd-flex', 'align-items-center', 'gap-2', 'mb-1');
            propertyTitle.classList.add('form-label', 'mb-0'); // mb-0 perquè ja fa mb-1 el header
            ...
            controlStack.classList.add('d-flex', 'align-items-center', 'gap-2', 'flex-nowrap', 'content-field');
    ...
    makeRowValueElement(id, name, property) {
        ...
        switch (property.type) {
            ...
            case 'select':
                ...
                for (let [value, text] of Object.entries(property.options || {})) {
                    ...
                    if (value === property.value) optionElement.setAttribute('selected', 'selected');
        ...
        valueElement = this.addAttributesRowValueElement(id, name, property, valueElement);
        ...
        switch (property.type) {
            ...
            case 'textarea':
                ...
                valueElement.setAttribute('rows', property.rows ? property.rows : 3);
                break;
            case 'checkbox':
                break;
            case 'date':
    ...
    addAttributesRowValueElement(id, name, property, valueElement) {
        ...
        valueElement.setAttribute('category', property.category ? Object.keys(property.category)[0] : '');
        valueElement.setAttribute('group', property.groups ? Object.keys(property.groups).pop() : '');
        ...
        if (property.onchange) {
            valueElement.addEventListener('change', () => {
                ...
                switch (property.type) {
                    case 'select':
                        value = valueElement.selectedOptions[0].innerHTML.trim();
                        break;
                    default:
                        value = valueElement.value.trim();
                        break;
                }
                this.propertiesFormElement.querySelector(`#${property.onchange} input`).value = value;
            });
        }
        return valueElement;
    }
    ...
    makeRowElementHelp(property) {
        if (property.help) {
            ...
            helpContainer.classList.add('exe-form-help', 'help-content-disabled');

            const helpIcon = document.createElement('span');
            helpIcon.classList.add('form-help-exe-icon', 'icon-medium', 'info-icon-solid-green');
    ...
    makeRowActionsElement(property, row) {
        ...
        if (property.duplicate) {
            ...
            actionsContainer.classList.add('actions-duplicate-properties-container', 'd-inline-flex', 'align-items-center', 'gap-2');
            ...
            this.addEventClickToActionAddButton(actionAdd, property.duplicate, property, row);
            ...
            this.addEventClickToActionDeleteButton(actionDelete, property.duplicate, property, row);
    ...
    addEventClickToActionAddButton(button, duplicate, property, row) {
        button.addEventListener('click', () => {
            ...
            rowsToDuplicate.reverse().forEach((cloneRowElement) => {
                ...
                cloneRowElement.querySelectorAll('.actions-duplicate-properties-container').forEach((actionsContainer) => {
                    const prevElement = actionsContainer.parentNode.previousSibling;
                    if (actionsContainer && prevElement) {
                        const showRemoveButton =
                            prevElement.classList.contains('property-row') ||
                            cloneRowElement.classList.contains('first-copied-row');
                        actionsContainer.setAttribute('original', !showRemoveButton);
                    }
                });
            });
        });
    ...
    addEventClickToActionDeleteButton(button, duplicate, property, row) {
        button.addEventListener('click', () => {
            ...
            const propertyId = row.querySelector('.property-value').getAttribute('property');
            const isOriginal = button.parentNode.getAttribute('original') == 'true';
            const propertiesRows = container.querySelectorAll(`.property-value[property="${propertyId}"]`);
            ...
            if (nPropertiesRows > 1 && !isOriginal) {
                ...
                let nextRow = row;
                for (let i = 0; i < duplicate; i++) {
                    if (nextRow && nextRow != row && nextRow.classList.contains('first-copied-row')) break;
                    rowsToDelete.push(nextRow);
                    if (i < duplicate - 1) nextRow = nextRow.nextElementSibling ? nextRow.nextElementSibling : false;
                }
                rowsToDelete.forEach((rowElement) => { if (rowElement) rowElement.remove(); });
            }
        });
    }
    ...
    cloneRowElement(row, propertyBase, num) {
        ...
        if (childrenLabel) {
            ...
            childrenLabel.addEventListener('click', () => { childrenValue && childrenValue.focus(); });
        }
        if (childrenValue) {
            childrenValue.id = newId;
            childrenValue.setAttribute('name', newId);
            childrenValue.setAttribute('groupCopied', propertyBase.groups ? Object.keys(propertyBase.groups).pop() : '');
            if (childrenValue.tagName.toLowerCase() !== 'select') childrenValue.value = '';
        }
        ...
        const actionsContainer = cloneRow.querySelector('.actions-duplicate-properties-container');
        if (actionsContainer) {
            const nDuplicate = actionsContainer.getAttribute('duplicate');
            const buttonAdd = actionsContainer.querySelector('.add-properties');
            const buttonDelete = actionsContainer.querySelector('.delete-properties');
            this.addEventClickToActionAddButton(buttonAdd, nDuplicate, property, cloneRow);
            this.addEventClickToActionDeleteButton(buttonDelete, nDuplicate, property, cloneRow);
        }
    ...
    saveAction() {
        ...
        if (missingFields) {
            ...
            this.saveProperties(propertiesDict, false).then((response) => {
                this.properties.project.app.locale.loadContentTranslationsStrings(propertiesDict.pp_lang);
                const toastData = {
                    title: _('Project properties'),
    ...
    getPropertiesData() {
        ...
        const propertiesValueElements = this.nodeContent.querySelectorAll('.property-value');
        propertiesValueElements.forEach((propertyValue) => {
            const property = this.metadataProperties[propertyValue.getAttribute('property')];
    ...
    getPropertyKeyMultiple(propertyValue, property, propertyKeyBase, propertiesGroupNum, propertiesDict) {
        ...
        for (const group of Object.keys(property.groups || {})) {
            ...
            if (group == propertyKeyPrefixGroup) {
                ...
                const numStringSuffix = propertiesGroupNum[propertyPre] ? propertiesGroupNum[propertyPre] : '';
                const groupSuffix = propertyKeyBase.replace(propertyKeyPrefixGroup, '');
                const propertyKeyPrototipe = propertyKey + groupDiff + numStringSuffix + groupSuffix;
                if (propertiesDict[propertyKeyPrototipe] !== undefined) {
                    const numGroup = propertiesGroupNum[propertyPre] ? propertiesGroupNum[propertyPre] + 1 : 2;
                    propertiesGroupNum[propertyPre] = numGroup;
                }
                const numGroupString = propertiesGroupNum[propertyPre] ? propertiesGroupNum[propertyPre] : '';
                propertyKey += groupDiff + numGroupString + groupSuffix;
            } else {
                const numStringGroup = propertiesGroupNum[propertyKey] ? propertiesGroupNum[propertyKey] : '';
    ...
    makeSaveButton() {
        ...
        buttonSave.classList.add('confirm', 'btn', 'btn-primary');
    ...
    addBehaviourSaveButton() {
        const saveButton = this.propertiesFormElement.querySelector('button.confirm');
        saveButton.addEventListener('click', () => this.saveAction());
    }

    addBehaviourTextInputs() {
        this.propertiesFormElement.querySelectorAll('input[type="text"]').forEach((input) => {
            input.addEventListener('keyup', (event) => {
                event.preventDefault();
                if (event.key == 'Enter') this.saveAction();
            });
    ...
    addBehaviourExeHelp() {
        ...
        exeHelp.forEach((help) => {
            ...
            help.querySelector('.form-help-exe-icon').addEventListener('click', () => {
    ...
    addBehaviourBodyToHideHelpDialogs() {
        document.querySelector('body').addEventListener('click', (event) => {
            if (!event.target.classList.contains('form-help-exe-icon')) this.hideHelpContentAll();
        });
    }
    ...
    reloadValues() {
        ...
        for (let [key, property] of Object.entries(this.metadataProperties)) {
            ...
            if (property.multipleId) {
                ...
                element = this.nodeContent.querySelector(`.property-value[property="${key}"]`);
            }
            if (!element) continue;
            switch (property.type) {
                case 'checkbox':
                    ...
                    break;
                case 'select': {
                    const select = element.querySelector(`option[value="${property.value}"]`);

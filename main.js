...
// macOS flag for native tabbed windows
...
// Small preferences helper (persisted in settings.json)
...
function attachOpenHandler(win) {
  win.webContents.setWindowOpenHandler(({ url }) => {
    ...
    // Windows/Linux → open in a separate window
...
/**
 * On macOS, ensure tab/window closing is never blocked by beforeunload handlers.
 * Calling preventDefault() here ALLOWS the unload to proceed.
 */
function bindMacCloseSafety(win) {
  if (!win || process.platform !== 'darwin') return;

  win.webContents.on('will-prevent-unload', (event) => {
    // Allow the page to close even if it tries to prevent unload
    event.preventDefault();
  });

  // Do not call preventDefault on 'close' for macOS windows/tabs.
  win.on('close', () => {});
}

/**
 * Create a BrowserWindow that joins the native tab group on macOS.
 */
function createMacTab(url, title = 'Document') {
  ...
  const win = new BrowserWindow({
    width: 1200,
    height: 800,
    show: false,                    // show after joining as a tab
    tabbingIdentifier: 'mainGroup', // native tab group id
    webPreferences: {
      nodeIntegration: false,
      contextIsolation: true,
      preload: path.join(__dirname, 'preload.js'),
  ...
  win.once('ready-to-show', () => {
    try {
      if (parent && !parent.isDestroyed()) parent.addTabbedWindow(win);
    } catch (_) {}
    win.show();
  });

  // Keep window.open behavior consistent for tabs as well
...
function createWindow() {
  ...
  waitForServer(() => {
    ...
    if (isMac) {
      bindMacCloseSafety(mainWindow);

      // Ensure the native tab bar becomes visible at least once
